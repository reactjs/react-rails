;(function(document, window) {
  ...
  function subscribeStore(node, constructor, props) {
    if (typeof(props.reduxStore) !== 'undefined') {
      if (typeof(window.Store) === 'undefined') {
          var error = new Error("Make sure you have set window.Store to 'redux/store'");
          throw error
      } else {

        window.Store.subscribe(function () {
          const storeState = window.Store.getState();
          const specificStoreState = storeState[props.reduxStore];

          if (typeof(specificStoreState) === 'undefined') {
              var error = new Error("Make sure your Store is globally available");
              throw error
          } else {

            const newProps = Object.assign({}, props, specificStoreState);

            ReactDOM.render(
              React.createElement(constructor, newProps),
              node
            );
          }
        });
    ...
    mountComponents: function(searchSelector) {
      ...
      for (var i = 0; i < nodes.length; ++i) {
        ...
        if (typeof(constructor) === "undefined") {
          ...
          ReactDOM.render(React.createElement(constructor, props), node, () => subscribeStore(node, constructor, props));
        }
      }
    },
